...
/**
 * Checks if a workspace is locked from renaming
 */
...
function isLocked(
  ...
  const { lockedSymbol } = options
  if (!lockedSymbol) return false

  if (workspace.name.includes(lockedSymbol)) return true
  return false
}
