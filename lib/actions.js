...
function autoRename(
  ...
  const workspaces = query(
    root,
    node => node.type === 'workspace' && node.output !== '__i3'
  )
  let result = []
  let number = 0
...
function getFocusedWindow(workspace) {
  ...
  if (workspace.focus) {
    const node /*: ?ContainerWindow */ = workspace.focus.reduce(
      (result, id) =>
        result || find(workspace, node => node.id === id && node.window),
      null
    )
...
function renameMsg(
  ...
  let focusedName = node && getClassName(node)
  let newName
